...
/**
 * Collapsible content using HTML text.
 *
 * @package    mod_mubook
 * @copyright  2025 Petr Skoda
 * @license    https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class collapsible extends \mod_mubook\local\content {
    #[\Override]
    public function render(\renderer_base $output, toc $toc, int $firstheading, int $headingoffset = 0): string {
        ...
        if (trim($this->data2 ?? '') === '') {
            $label = get_string('content_type_collapsible_label_default', 'mod_mubook');
        } else {
            $label = format_string($this->data2);
        }

        $data = [
            'editing' => $output->get_page()->user_is_editing(),
            'contentid' => $this->id,
            'label' => $label,
            'html' => $html,
        ];

        return '<div class="mubook-format-html no-overflow">' . $output->render_from_template('mod_mubook/content/collapsible', $data) . '</div>';
    }
